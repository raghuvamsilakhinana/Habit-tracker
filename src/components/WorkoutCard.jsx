...
export default function WorkoutCard({ plan, workoutDay, dateKey, session, sessionSets, available, onOpenPlan, onCustomizePlan, onStart, onHistory }) {
  if (!available) {
    return (
    ...
    <section className="workout-card animate-fade-in mb-5">
      <div className="relative p-5 sm:p-6">
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
          <div className="min-w-0">
            <div className="workout-kicker"><span>🏋️</span> Today's workout · {formatWorkoutDate(dateKey, { year: undefined })}</div>
          ...
          <div className="flex flex-wrap gap-2"><button className="workout-primary-btn" onClick={onStart}>{session ? (isComplete ? 'Review workout' : 'Resume workout') : 'Start workout'} <span>→</span></button><button className="workout-secondary-btn" onClick={onOpenPlan}>Change routine</button><button className="workout-secondary-btn" onClick={onCustomizePlan}>Customize</button><button className="workout-secondary-btn" onClick={onHistory}>History</button></div>
        </div>
